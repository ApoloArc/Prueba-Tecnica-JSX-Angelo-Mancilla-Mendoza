...
function Cart() {
  ...
  function calcularTotal() {
    ...
    store.carrito.pedidoCompleto.forEach(pedido => {
      result = Number(
        (pedido.producto.price * pedido.cantidad + result).toFixed(2)
      )
    })
  ...
  function eliminarProducto(paraEliminar) {
    if (store.carrito.pedidoCompleto.length === 1) {
      ...
      const nuevaStore = store.carrito.pedidoCompleto.filter(
        producto => producto.producto.name !== paraEliminar.producto.name
      )
      store.setCarrito({
                  ...
                  {store.carrito.pedidoCompleto.map(producto => (
                    <div className={style.cart__containerAll}>
                      <div className={style.cart__containerImgAndText}>
                        ...
                        <img
                          className={style.cart__imgProduct}
                          src={store.url + producto.producto.image}
                          alt="img-store"
                        />
                      ...
                      <div className={style.cart__divPriceAndQuantity}>
                        ...
                        <div>
                          ...
                          <input
                            className={style.cart__inputNumber}
                            type="number"
