...
function CustomButton(props: data) {
  ...
  const [carritoActualizado, setCarritoActualizado] = useState<ICarrito>(() => {
    try {
  ...
  function notify(message: string, producto: IProduct) {
    ...
    const existe = store.carrito.pedidoCompleto.find((pedido) => pedido.producto.name === producto.name);
    ...
    if (existe !== undefined) {
      store.carrito.pedidoCompleto.forEach((pedido, i) => {
        if (pedido.producto.name === producto.name) {
          try {
            ...
            store.carrito.pedidoCompleto[i].cantidad = props.cantidadPedido ? props.cantidadPedido + existe.cantidad : existe.cantidad + 1;
            setChange(!change);
      ...
      try {
        store.setCarrito!({
          pedidoCompleto: [...carritoActualizado.pedidoCompleto, { producto, cantidad: props.cantidadPedido ? props.cantidadPedido : 1 }],
        });
        return toast.success(message);
      } catch (error) {
        console.error(error);
  ...
  return (
    <>
      <button disabled={props.disabled} style={props.style} onClick={() => notify("Product added ❤️", props.producto)} className="button">
        Add item to cart
      </button>
      <Toaster />
    </>
